...
export function AnalyticsDashboard({ dateRange }: AnalyticsDashboardProps) {
  ...
  const [analyticsData, setAnalyticsData] = useState<any>({
    ...
    orderStatus: [],
    revenueByCategory: [],
    summary: null, // Add summary to state
    recentOrders: [], // Add recentOrders to state
  })
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Merge real summary with mock chart data for now
        ...
        // Keep mock data for charts until implemented
  ...
  useRealtimeDashboard(() => {
    // Re-fetch data on realtime update
    const fetchData = async () => {
      try {
        const response = await fetch('/api/admin/analytics')
        if (!response.ok) throw new Error('Failed to fetch analytics')
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {analyticsData.summary?.totalRevenue?.toLocaleString('ka-GE', {
                style: 'currency',
                currency: 'GEL',
              }) || '₾0.00'}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{analyticsData.summary?.totalOrders || 0}</div>
            <p className="text-xs text-muted-foreground">+180 ბოლო საათში</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{analyticsData.summary?.activeUsers || 0}</div>
            <p className="text-xs text-muted-foreground">+19 ახალი დარეგისტრირებული</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">საშ. შეკვეთის ღირებულება</CardTitle>
            <TrendingUp className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {analyticsData.summary?.averageOrderValue?.toLocaleString('ka-GE', {
                style: 'currency',
                currency: 'GEL',
              }) || '₾0.00'}
            </div>
            <p className="text-xs text-muted-foreground">+4% წინა კვირასთან შედარებით</p>
          </CardContent>
        </Card>
      </div>
