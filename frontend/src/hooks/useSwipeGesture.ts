...
export function useSwipeGesture<T extends HTMLElement = HTMLElement>(options: SwipeGestureOptions) {
  ...
  const triggerHaptic = () => {
    if (hapticFeedback && 'vibrate' in navigator && typeof navigator.vibrate === 'function') {
  ...
  }, [onSwipeLeft, onSwipeRight, onSwipeUp, onSwipeDown, minSwipeDistance, hapticFeedback])
